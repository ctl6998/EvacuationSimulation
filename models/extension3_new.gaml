/**
* Name: evacuation - Extension 3 with intersection strategy
* Author: ctl
* Tags: 
*/
...
global {
    ...
    string awareness_strategy <- "random" among: ["random", "furthest", "closest", "intersection"];
    ...
    // New variable to store major intersections
    int number_intersection <- 10;
    int min_connecting_roads <- 4;
    float intersection_range <- 50.0 #m;
    list<point> major_intersections;
	...
	list<point> find_major_intersections {
    	// Create a map to store intersection points and their connection counts
    	map<point, int> intersection_counts;
    
    	// Count road connections at each intersection point
    	ask road {
        	point start_point <- first(shape.points);
        	point end_point <- last(shape.points);
        
        	// Count connections at start point
        	if(intersection_counts.keys contains start_point) {
        	    intersection_counts[start_point] <- intersection_counts[start_point] + 1;
        	} else {
        	    intersection_counts[start_point] <- 1;
        	}
        
        	// Count connections at end point
        	if(intersection_counts.keys contains end_point) {
         	   intersection_counts[end_point] <- intersection_counts[end_point] + 1;
        	} else {
        	    intersection_counts[end_point] <- 1;
        	}
    	}
    
	    // Filter to keep only points withenought connections
	    list<point> major_points <- intersection_counts.keys where (intersection_counts[each] >= min_connecting_roads);
    
    	// Randomly select from the major intersections
    	return number_intersection among major_points;
	}
    ...
    init {
        ...
        // Find major intersections before applying awareness strategy
        major_intersections <- find_major_intersections();
        ...
        switch awareness_strategy {
            ...
            match "intersection" {
    			// Make people near major intersections aware
    			loop intersection over: major_intersections {
        			ask inhabitant {
            			if (self.location distance_to intersection) < intersection_range {
                		is_aware <- true;
                		knows_shelter_location <- flip(0.1);
            			}
        			}
...
species road {
   aspect default {
       ...
       draw shape + (thickness * 1.5) color: rgb(base_color.red, base_color.green, base_color.blue, 200);
       draw shape + (thickness * 2) color: rgb(base_color.red, base_color.green, base_color.blue, 128);
   }
}
...
experiment single_run type: gui {
    parameter "Awareness Strategy" var: awareness_strategy <- "random" among: ["random", "furthest", "closest", "intersection"];
    ...
    output {
        display map type: 3d {
            ...
            // Visualize the major intersections and their 50m radius
            graphics "intersection_zones" {
                loop intersection over: major_intersections {
                    // Draw intersection point
                    draw circle(5) at: intersection color: #yellow border: #black;
                    // Draw 50m radius circle
                    draw circle(intersection_range) at: intersection color: rgb(255,255,0,50) border: rgb(255,255,0,100);
                }
            }
        }
    ...
    permanent {
        display ComparisonCharts background: #white {
            chart "Efficiency by Strategy" type: histogram background: #white position: {0, 0} size: {1.0, 1.0} {
                ...
                data "Intersection" value: mean(simulations where (each.awareness_strategy = "intersection") collect 
                    (each.total_evacuation_time / each.total_road_time)) color: #yellow;
            }
        }
    }
}